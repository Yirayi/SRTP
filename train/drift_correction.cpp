...
string inputPath = "2.mp4";
string outputPath = "after.mp4";
...
int main(int argc, char* argv[])
{
	...
	Mat startImage = imread("data4correction\\data\\test2_after\\1_pre.png", CV_LOAD_IMAGE_GRAYSCALE);   //第一帧
	Mat backgroundImage = imread("data4correction\\data\\test2_after\\1_pre.png"); //画图背景
	char image_name[200];
	...
	imwrite("data4correction\\data\\cornerdetect2.png", backgroundImage);
	
	for (int i = 2; i <= 298; i++)
	{
		sprintf_s(image_name, "data4correction\\data\\test2_after\\(%d)_pre.png", i);
		...
		sprintf_s(result_name, "data4correction\\data\\test2_cor\\drift(%d).tif", i);
	...
	imshow("result", backgroundImage);
	imwrite("data4correction\\data\\\\aaa.png", backgroundImage);
	waitKey(0)
}
